"""
visualise_herds_10km.py  —  Descriptive stats + maps for 10 km convoy-threshold herds.
Reads herds_10km.csv produced by find_herds.py --spatial-threshold 10.

Outputs (all written to maps_10km/):
  ...
  03_world_large_herds.png        – herds with 3+ vessels highlighted
...
INPUT  = Path("herds_10km.csv")
OUTDIR = Path("maps_10km")
...
base_map(ax, title="Convoy Herds — Coloured by Duration of Co-Travel (10 km threshold, within visual/signal range)")
...
large = df[df["n_vessels"] >= 3].copy()

fig, ax = plt.subplots(
    figsize=(16, 8),
    subplot_kw={"projection": ccrs.Robinson()},
)
base_map(ax, title=f"Convoy Herds — 3+ Vessels within 10 km  (n={len(large):,})")
